#' @title Individual Ridge Global Frechet Regression for Densities with Wasserstein metric
#' 
#' @description Individual Ridge Glocal Frechet regression with respect to the Wasserstein distance.
#' 
...
#' @param lambda A p by 1 vector of inverse ridge regularization parameter
...
IndivRidgeGloWassReg = function(xin, Qin, xout, lambda, lower=NULL, upper=NULL, Rsquared=FALSE, Qgrid=NULL){
  ...
  # New weights according to individually penalized ridge Frechet regression  
  NewInvSigma=diagsqrtlambda%*%solve(diagsqrtlambda%*%Sigma%*%diagsqrtlambda+diag(length(lambda)))%*%diagsqrtlambda
  
  ### Here is where R hat is computed ### (See section 3.1.1 of the supplementary materials)
  Qout = sapply(1:k, function(j){
    s = 1 + t(t(xin) - xbar) %*% NewInvSigma %*% (xout[j,] - xbar)
    s = as.vector(s)
    gx = colMeans(Qin * s)
  ...
  if (!Rsquared) {
    ...
    if (k == n) {
      ...
      Qin.est = sapply(1:n, function(j){
        s = 1 + t(t(xin) - xbar) %*% NewInvSigma %*% (xin[j,] - xbar)
        s = as.vector(s)
        gx = colMeans(Qin * s)
        res = do.call(quadprog::solve.QP, list(diag(m), gx, A, b0))
        # return(sort(res$solution))
