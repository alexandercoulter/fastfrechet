...
Fréchet regression generalizes Euclidean regression to the general metric space setting. In the sample setting, we observe covariate-response pairs $\{(\mathbf{x}_i, \mathbf{y}_i)\} \subset \mathbb{R}^p \times \Omega$, where $\Omega$ is the space of univariate quantile functions equipped with the 2-Wasserstein metric $d_W(\mathbf{q}, \mathbf{p}) = \lVert\mathbf{q} - \mathbf{p}\rVert_{L_2[0,1]}$. (Generally we also only observe the distributions over a discrete grid, not as full functions.) We treat the covariate vectors $\mathbf{x}_i$ as rows of matrix $\mathbf{X}$, which is column-centered and scaled so that $\mathrm{diag}(\mathbf{X}^{\top}\mathbf{X}) = \pmb{1}$. The conditional Fréchet mean is estimated by
$$
\widehat{\mathbf{q}}_{\oplus}(\mathbf{x}_i) := \underset{\mathbf{q}\in\Omega}{\mathrm{argmin}} \sum_{j=1}^n \left\{ \frac{1}{n} + \mathbf{x}_i^{\top}(\mathbf{X}^{\top}\mathbf{X})^+\mathbf{x}_j\right\} d_W^2(\mathbf{y}_j, \mathbf{q}).
$$
...
The Fréchet regression problem has unique solution even when the model matrix $\mathbf{X}$ is not full rank.  The weight matrix $\mathbf{Z}(\mathbf{X}^{\top}\mathbf{X})^-\mathbf{X}^{\top}$ is invariant to choice of generalized inverse $(\mathbf{X}^{\top}\mathbf{X})^-$
$$

$$`fastfrechet` implements Fréchet regression even when the model matrix $\mathbf{X}$ is not full rank.  
