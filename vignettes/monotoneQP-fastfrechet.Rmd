...
accompanying `intro-fastfrechet` vignette for details.) This is a quadratic
programming (QP) problem, where the final projected vector
$\widehat{\mathbf{q}}$ is monotone non-decreasing (i.e. $q_i \leq q_j$ for
$i \leq j$) and obeys the same box constraints as the $\mathbf{y}$'s. While the
`fastfrechet` package solves the Fréchet regression problem via the
`frechetreg_univar2wass` function, it also implements a solver for the more
general QP problem of finding the closest "monotone and bounded" vector(s) via
the `monotoneQP` function.
