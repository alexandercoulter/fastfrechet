...
which does not necessarily sit in the target space. The final projected vector
$\widehat{\mathbf{q}}$ is monotone non-decreasing (i.e. $q_i \leq q_j$ for
$i \leq j$) and obeys the same box constraints as the $\mathbf{y}$'s. The
`fastfrechet` package implements a solver for this problem via the `monotoneQP`
function.
...
Solving for $\widehat{\mathbf{q}}$ is a quadratic programming (QP) problem, for
which several approaches and solvers exist. We introduce a modification of the
dual active-set method of Arnström et al. (2022, Algorithm 1) which utilizes the
sparse structure of $\mathbf{A}$ to avoid costly matrix multiplication and
$LDL^{\top}$ decomposition steps. This algorithm is implemented in *C++* as a
sub-routine in `fastfrechet` functions solving Fréchet mean problems for
2-Wasserstein space, and is also made available as a standalone QP solver in the
`monotoneQP` function.

### Notation

The dual active-set algorithm of Arnström et al. (2022) is for QP problems with
...
The full Algorithm 1 from Arnström et al. (2022) is given below, with applicable
...
The implementation of the dual active-set method from Arnström et al. (2022)
