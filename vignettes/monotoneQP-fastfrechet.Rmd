...
This vignette details the mathematical foundation of the custom dual active-set
method we implement to solve the Fréchet mean problem for univariate
distribution responses equipped with the 2-Wasserstein metric. In the
"population setting", this is an orthogonal projection problem. In the "sample
setting" where distribution responses are represented by quantile functions
...
$$
\widehat{\mathbf{q}} := \mathrm{arg\:min}_{\mathbf{q} \in \mathbb{R}^m} \lVert{\mathbf{q} - \mathbf{y}} \rVert_2^2, \qquad \mathbf{b} - \mathbf{A}^{\top}\mathbf{q} \leq \pmb{0}_{m+1},
$$
...
approaches and solvers exist. We introduce a modification of the dual active-set
method of Arnström et al. (2022, Algorithm 1) which utilizes the structure of
$\mathbf{A}$ to avoid costly matrix multiplication and $LDL^{\top}$
decomposition steps. This algorithm is implemented in *C++* as a sub-routine in
functions handling responses in this metric space, and is made available in its
own *R* wrapper function `monotoneQP`.
...
algorithm that takes advantage of the nearly-unique diagonal entries of
