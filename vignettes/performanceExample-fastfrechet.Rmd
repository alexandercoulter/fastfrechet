...
s = system.file("Tucker_code", "GlobalObj4h.R", package = "fastfrechet")
if(file.exists(s)) source(s)
s = system.file("Tucker_code", "GloWassReg.R", package = "fastfrechet")
if(file.exists(s)) source(s)
s = system.file("Tucker_code", "IndivRidgeGloWassReg.R", package = "fastfrechet")
if(file.exists(s)) source(s)
s = system.file("Tucker_code", "lambdaGlobalcoordesc.R", package = "fastfrechet")
if(file.exists(s)) source(s)
```
...
`fastfrechet` and these othr implementations, using an example simulated data
...
zero-inflated negative binomial distributions. The `fastfrechet` function
`generate_zinbinom_qf` allows us to simulate $n$ such distributions, represented
as quantile functions evaluated on a shared $m$-grid in $(0, 1)$, dependent on
the first 4 of $p \geq 4$ covariates. The example data set we generate for this
vignette has sample size $n = 100$, covariate number $p = 10$, and quantile
functions evaluated on an equidistant $m = 100$ grid.
...
file.copy("zinb_examples.png",
          file.path(rprojroot::find_rstudio_root_file(), "inst", "doc", "zinb_examples.png"),
          overwrite = TRUE)
```
...
the `WRI` package can fit solutions in the low-rank setting, however it does not
...
The implementations from `frechet` and the Tucker materials allow non-increasing
distribution inputs and user-specified box constraints. However, neither method
can solve the regression problem in the low-rank setting; the former is
relatively slow, even for small $m$; and the latter is not currently available
in an R package.
...
M1 = microbenchmark("WRI" = wass_regress( ~ ., X_df, "quantile", Y_adj),
...
```{r, echo = FALSE}
# Load times:
s = system.file("doc", "microbenchmark_results.Rda", package = "fastfrechet")
...
text(par("usr")[1] + 0.17, par("usr")[4] + 0.0235, col = 'gray40',
...
text(par("usr")[1] + 0.32, par("usr")[4] + 0.0235, col = 'gray40',
     "R PACKAGE", pos = 4, xpd = TRUE, cex = 0.45, font = 2)
text(par("usr")[2] + 0.01, par("usr")[3] + 0.06,
...
text(par("usr")[1] + 0.435, par("usr")[4] + 0.0235, col = 'gray40',
     "R PACKAGE", pos = 4, xpd = TRUE, cex = 0.45, font = 2)
text(par("usr")[2] + 0.01, par("usr")[3] + 0.06,
...
file.copy("frechetreg_accuracy_comparison.png",
          file.path(rprojroot::find_rstudio_root_file(), "inst", "doc", "frechetreg_accuracy_comparison.png"),
          overwrite = TRUE)
```
...
# Benchmark times over 5 iterations:
M2 = microbenchmark("Tucker" = FRiSO_Tucker(X0, Y, 0, 1e6, tauseq),
...
mais = cbind(c(0.35, 0.35, 0.35),
             c(0.4, 0.04, 0.55),
             c(0.2, 0.2, 0.2),
             c(0.04, 0, 0.08))
figs = cbind(c(0, 0.4 + a + 0.04, 0.4 + a + 0.08 + a) / 6.5,
             c(0.4 + a + 0.04, 0.4 + a + 0.08 + a, 6.5) / 6.5,
             c(0, 0, 0),
             c(1, 1, 1))
# Save graph:
png("friso_accuracy_comparison.png", width = 6.5, height = a + 0.35 + 0.2, units = "in", res = 576)

par(mai = mais[1, ], fig = figs[1, ], family = "Helvetica")
plot(x = c(), y = c(), xlim = c(-0.1, 10.1), ylim = c(0, 4), axes = FALSE,
     xlab = "", ylab = "")
polygon(x = par("usr")[c(1, 2, 2, 1)], y = par("usr")[c(3, 3, 4, 4)],
        col = "gray93", border = NA)
abline(h = axTicks(2), v = axTicks(1), col = "white")
...
text(par("usr")[1] - 0.6, par("usr")[4] + 0.15,
     "Tucker", pos = 4, xpd = TRUE, cex = 0.75, font = 2)
text(par("usr")[1] + 2.3, par("usr")[4] + 0.1175, col = 'gray40',
     "PAPER SUPPLEMENT", pos = 4, xpd = TRUE, cex = 0.45, font = 2)
...
text(par("usr")[1] - 0.6, par("usr")[4] + 0.15,
     "fastfrechet", pos = 4, xpd = TRUE, cex = 0.75, font = 2)
text(par("usr")[1] + 3.7, par("usr")[4] + 0.1175, col = 'gray40',
     "R PACKAGE", pos = 4, xpd = TRUE, cex = 0.45, font = 2)
...
plot(x = c(), y = c(), xlim = c(-0.1, 10.1), ylim = c(0.9999, 1.0001), axes = FALSE,
     xlab = "", ylab = "")
polygon(x = par("usr")[c(1, 2, 2, 1)], y = par("usr")[c(3, 3, 4, 4)],
        col = "gray93", border = NA)
abline(h = axTicks(2), v = axTicks(1), col = "white")
abline(h = 1, lty = 2, col = "gray50")
matlines(tauseq, f_fastfrechet / f_Tucker, type = 'o', lty = 1,
         col = c("black", "#648FFF", "#FFB000", "#DC267F"),
         lwd = 1, pch = 15:18, cex = c(0.5, 0.5, 0.5, 0.7))
axis(1, NULL, TRUE, FALSE, -1.3, cex.axis = 0.55)
axis(2, c(0.9999, 1, 1.0001),
     c(expression("1 \U2013 10"^-4), expression("1.0"), expression("1 + 10"^-4)), FALSE, -0.9, las = 1, cex.axis = 0.55)
mtext(expression(tau), 1, 0.7, cex = 0.8)
mtext(expression("f("*bold(widehat(lambda))*")"["fastfrechet"]*" / f("*bold(widehat(lambda))*")"[Tucker]),
      2, 1.3, las = 0, cex = 0.8)
legend("topleft", col = c("black", "#648FFF", "#FFB000", "#DC267F"),
...
file.copy("friso_accuracy_comparison.png",
          file.path(rprojroot::find_rstudio_root_file(), "inst", "doc", "friso_accuracy_comparison.png"),
          overwrite = TRUE)
```
